<?php
/*
Plugin Name:[GWA] Database Editor
...
/*
    Copyright 2010  G.J.P  (email : cookies@getwebactive.com)
...
<div class="wrap">
		<div id="content">
			...
			<br /><hr style="color:#666"></div>
      <p>This is a <strong style="color:#f00">DANGEROUS & POWERFUL</strong>  direct database editor utilizing <a href="http://www.millstream.com.au/view/code/tablekit" target="_blank">TableKit</a> and AJAX for fast and easy data updates.</p>
      <p>You can quickly destroy your weblog using this plugin. <span style="color:#f00"><em>If you don't know what you are doing UNINSTALL THIS SOFTWARE NOW!</em></span></p>
<?php
}
...
headout();
sqlbox();
echo '<br /><CENTER><textarea cols=80 rows=40>';
print_r($result);
echo '</textarea></CENTER><br /><br />';
} else
if($_POST['db_gwa_agree']) {
add_option('db_gwa_agree',1);
headout();
sqlbox();
} else
if(!get_option('db_gwa_agree')) {
headout();
?>
      <CENTER><form action="" method="POST"><input type="submit" value="I ACCEPT THE RISK OF USING THIS PLUGIN!" name="db_gwa_agree"></form></CENTER><?php
} else
if(isset($_POST['db_gwa_selected']) && get_option('db_gwa_agree')) {
...
<div class="wrap">
		...
		<div id="content">
			<h1 style="position:relative;float:left;width:300px;">[GWA] db Editor</h1>
			<?php db_gwa_build_table_list(); ?>
			<br /><hr style="color:#666">Click a cell (value) to update the field. Click 'Ok' to commit changes to the database. UPDATES ARE IRREVERSIBLE!<hr style="color:#666">
			<table class="sortable resizable editable">
				<thead>
					<tr>
<?php          
for($i=0;$i<count($col);$i++)
echo '          <th class="sortfirstdesc" id="'.$col[$i].'" width="10%">'.$col[$i].'</th>';
?>        
          </tr>
				</thead>
				<tbody>
...
function sqlbox() {
?>
<CENTER><form action="" method="POST" name="db_gwa_sql">Query Type: <select name="db_gwa_query_type"><option>Variable</option><option>Row</option><option>Query</option></select><br /><textarea cols=80 rows=5 name="db_gwa_sql_query"></textarea><br /><input type="submit" value="<<< Query >>>"></form></CENTER>
<?php
}
...
if($_REQUEST['page']=='gwa-db-editor/db_gwa.php')
